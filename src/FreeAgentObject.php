...
class FreeAgentObject implements \ArrayAccess, \Countable, \JsonSerializable {
	...
	public function __set( $k, $v ) {
		if ( static::getPermanentAttributes()->includes( $k ) ) {
			throw new Exception\InvalidArgumentException( "Cannot set $k on this object. HINT: you can't set: " . join( ', ', static::getPermanentAttributes()
			                                                                                                                        ->toArray() ) );
		}
		...
		$this->_values[ $k ] = Util\Util::convertToFreeAgentObject( $v, $this->_opts );
	...
	public function &__get( $k ) {
		...
		if ( ! empty( $this->_values ) && array_key_exists( $k, $this->_values ) ) {
			...
			$message = "FreeAgent Notice: Undefined property of $class instance: $k. " . "HINT: The $k attribute was set in the past, however. " . "It was then wiped when refreshing the object " . "with the result returned by FreeAgent's API, " . "probably as a result of a save(). The attributes currently " . "available on this object are: $attrs";
			FreeAgent::getLogger()->error( $message );

			return $nullval;
		} else {
			$class = get_class( $this );
			FreeAgent::getLogger()->error( "FreeAgent Notice: Undefined property of $class instance: $k" );
	...
	public function refreshFrom( $values, $opts, $partial = false ) {
		...
		if ( $values instanceof FreeAgentObject ) {
	...
	public function updateAttributes( $values, $opts = null, $dirty = true ) {
		foreach ( $values as $k => $v ) {
			// Special-case metadata to always be cast as a FreeAgentObject
	...
	public function serializeParameters( $force = false ) {
		...
		foreach ( $this->_values as $k => $v ) {
			...
			//   3. Its value is a FreeAgentObject. A FreeAgentObject may contain modified
			//      values within in that its parent FreeAgentObject doesn't know about.
			//
			$original = array_key_exists( $k, $this->_originalValues ) ? $this->_originalValues[ $k ] : null;
			$unsaved  = $this->_unsavedValues->includes( $k );
			if ( $force || $unsaved || $v instanceof FreeAgentObject ) {
	...
	public function serializeParamsValue( $value, $original, $unsaved, $force, $key = null ) {
		...
		if ( $value === null ) {
			...
			if ( Util\Util::isList( $value ) ) {
				...
				return Util\Util::convertToFreeAgentObject( $value, $this->_opts )->serializeParameters();
			}
		} elseif ( $value instanceof FreeAgentObject ) {
	...
	/**
	 * Returns a pretty JSON representation of the FreeAgent object.
	 *
	 * @return string The JSON representation of the FreeAgent object.
	 */
	...
	/**
	 * Sets all keys within the FreeAgentObject as unsaved so that they will be
	 * included with an update when `serializeParameters` is called. This
	 * method is also recursive, so any FreeAgentObjects contained as values or
	...
	protected static function deepCopy( $obj ) {
		if ( is_array( $obj ) ) {
			...
			return $copy;
		} elseif ( $obj instanceof FreeAgentObject ) {
	...
	/**
	 * @return object The last response from the FreeAgent API
	 */
	...
	/**
	 * Sets the last response from the FreeAgent API
	 *
